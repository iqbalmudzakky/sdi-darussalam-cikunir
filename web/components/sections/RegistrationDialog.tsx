...
const triggerVariants = cva(["group inline-flex cursor-pointer items-center justify-center gap-2", "font-medium transition-colors"], {
  variants: {
    variant: {
      ...
      compact: "bg-brand-600 px-4 py-2 text-[14px] text-white hover:bg-brand-700",

      /* Tombol lebar di menu navigasi mobile. */
      block: "w-full bg-brand-600 px-4 py-3 text-[15px] text-white hover:bg-brand-700",
    },
  },
  defaultVariants: {
    variant: "primary",
 ...
 */
export function RegistrationFormDialog({ open, onOpenChange }: RegistrationFormDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="lg" className="border-brand-100 bg-paper">
        <DialogHeader>
          <DialogTitle className="font-display text-xl font-semibold text-ink-900">Formulir pendaftaran</DialogTitle>
        </DialogHeader>
 ...
 */
export function RegistrationTrigger({ children, variant, className, onClick }: RegistrationTriggerProps) {
  return (
    <button type="button" onClick={onClick} className={cn(triggerVariants({ variant }), className)}>
      {children}
    </button>
  );
}
 ...
 */
export default function RegistrationDialog({ children, variant, className }: RegistrationDialogProps) {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <>
      <RegistrationTrigger variant={variant} className={className} onClick={() => setIsOpen(true)}>
        {children}
      </RegistrationTrigger>
