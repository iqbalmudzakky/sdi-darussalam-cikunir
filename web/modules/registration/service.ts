...
import type { PpdbRegistrationListItem } from "./repository";

import { withDbLogging } from "@/modules/db/errors";
import { RATE_LIMIT_MAX_PER_HOUR, RATE_LIMIT_WINDOW_MINUTES, DUPLICATE_WINDOW_HOURS } from "@/modules/shared/constant/registration";

import type { PpdbRegistration, PpdbRegistrationStatus } from "./entity";

import type { CreatePpdbRegistrationRequest, CreatePpdbRegistrationResult, RegistrationListItemResponse } from "./dto";
...
function toRegistrationListItemResponse(item: PpdbRegistrationListItem): RegistrationListItemResponse {
  return {
    id: item.id,
...
export async function listRegistrations(): Promise<RegistrationListItemResponse[]> {
  const items = await withDbLogging("registration.list", () => repository.list());
